...
fun JobsTable.assertJobEquals(
    rows: List<ResultRow>,
    expected: List<SerializedJob<ExposedDatabaseQueueDriver.Metadata>>,
) {
...
fun JobsTable.assertJobEquals(
    row: ResultRow,
    expected: SerializedJob<ExposedDatabaseQueueDriver.Metadata>,
) {
    ...
    assertEquals(message = "attempts", actual = row[attempts], expected = expected.attempts)
