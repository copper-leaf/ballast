...
public abstract class LocalStateChannelInputStrategy<Inputs : Any, Events : Any, State : Any>(
    capacity: Int = Channel.BUFFERED,
    onBufferOverflow: BufferOverflow = BufferOverflow.SUSPEND,
) : LocalStateInputStrategy<Inputs, Events, State> {
    ...
    private suspend fun InputStrategyScope<Inputs, Events, State>.filterQueued(queued: Queued<Inputs, Events, State>): Boolean {
        when (queued) {
            ...
            is Queued.HandleInput -> {
                // when handling an Input with Local State, we don't have any state to reference for filtering, so we accept all
                return true
            }
